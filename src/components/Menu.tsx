...
const Menu = () => {
    ...
    function HandleMenu(event: React.MouseEvent<HTMLSpanElement>): void {
        ...
        if (!(event.target as HTMLSpanElement).innerHTML) return;
        switch ((event.target as HTMLSpanElement).innerHTML) {
            case "Read Me":
    ...
    return (
        <div className=' px-2 text-[#222] border-black border-b-2 selection:bg-transparent bg-white flex font-[ChicagoFLF] cursor-default'>
            {AlertElement}
            ...
            <div className='group/main  relative ' >
                ...
                <div className='absolute hidden group-hover/main:flex z-30
                 content-none top-[calc(100%)] left-0 bg-white text-[#222]
                 flex-col
                 border-2 border-[#222]
                 '>
                    {
                        LogoPages.map((page, i) =>
                            <div key={i} className='flex flex-col border-dotted border-b-2
                             border-gray-900 last:border-b-0 py-2 first:pt-0 last:pb-0'>
                                {
                                    page.map((p, i) =>
                                        <span key={i} onClick={HandleMenu} className='whitespace-nowrap py-[2px] px-2 cursor-pointer hover:bg-[#222] hover:text-white'>{p}</span>
                                    )
                                }
                            </div>)
                    }
